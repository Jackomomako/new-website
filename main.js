...
$(function () {
    ...
    ***************************/
    ...
    if (typeof gsap !== 'undefined' && typeof ScrollTrigger !== 'undefined') {
        ...
        // Select all elements with the 'mil-up' class
        const milUpElements = document.querySelectorAll('.mil-up');

        // Loop through each element and apply the animation
        milUpElements.forEach((element) => {
            gsap.fromTo(
                element,
                { opacity: 0, y: 50 }, // Start state: transparent and slightly below
                {
                    opacity: 1,
                    y: 0,
                    scrollTrigger: {
                        trigger: element, // Trigger animation when this element enters viewport
                        start: 'top 90%', // Animation starts when the element is near the bottom of the viewport
                        end: 'top 60%', // Optional: define when the animation completes
                        scrub: false, // No smooth scrub, animation runs naturally
        ...
        console.log("Animations initialized for .mil-up elements.");
    } else {
...
// Fade in preloader animation
timeline.to(".mil-preloader-animation", {
    opacity: 1,
});

// Animate first text block (Digital Marketing Specialists)
timeline.fromTo(
    ".mil-animation-1 .mil-h3", {
        ...
        y: "0px",
        opacity: 1,
        stagger: 0.4, // Stagger animation for individual lines
    }
);

// Fade out first text block
...
timeline.to(".mil-reveal-box", 0.45, {
    width: "100%", // Ensure it expands to the container width (not beyond the screen width)
    x: 0,
    ease: "sine",
}, "+=.1");

// Prevent overflow beyond the right side
timeline.to(".mil-reveal-box", {
    right: "0", // Ensure no overflow
    ease: "sine",
});

// Collapse the box back after animation
timeline.to(".mil-reveal-box", 0.3, {
    width: "0%", // Collapse back smoothly
    ease: "sine",
});

// Animate second text block (Sands Media)
...
timeline.fromTo(".mil-up", 0.8, {
    ...
    ***************************/
// Initialize Swiper only for screens wider than 768px
...
var swiper = new Swiper('.mil-infinite-show', {
    ...
    spaceBetween: 30,  // Space between slides (adjust for mobile)
    speed: 5000,
    autoplay: {
        delay: 0,
    },
    loop: true,
    freeMode: false,  // Free mode allows scrolling, might cause overflow on mobile
    breakpoints: {
        992: {
            slidesPerView: 4,  // For medium screens (tablets and larger)
        },
        768: {
            slidesPerView: 2,  // For smaller screens (portrait tablets)
            spaceBetween: 20,  // Adjust space between items
        },
        576: {
            slidesPerView: 1,  // For mobile screens (portrait mode)
